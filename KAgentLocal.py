...
class MyExampleAgent(Agent):
    ...
    async def get_move(self, game_state):
        ...
        # DeepSeek Prompt
        prompt = (
            f"You are a professional Gomoku player playing as {player_symbol}. Opponent is {rival_symbol}.\n"
            f"Think step-by-step internally but output only JSON.\n"
            f"Board:\n{board_str}\n\n"
            f"Candidate Moves:\n{moves_str}\n\n"
            f"Output: {{\"row\": <row>, \"col\": <col>}}"
        )

        self.log("Prompt to DeepSeek:\n" + prompt)

        response = self.llm_pipeline(prompt, max_length=512, temperature=0.2, do_sample=False)[0]['generated_text']
        self.log("DeepSeek Response:\n" + response)

        try:
            match = re.search(r"\{\s*\"row\"\s*:\s*\d+\s*,\s*\"col\"\s*:\s*\d+\s*\}", response)
            if match:
